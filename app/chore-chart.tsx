...
export function ChoreChart() {
  ...
  return <main className={`shell ${state.accessibilitySettings.highContrast ? "highContrast" : ""} ${state.accessibilitySettings.largeText ? "largeText" : ""} ${state.accessibilitySettings.reducedMotion ? "reduceMotion" : ""}`}>
    ...
    <section className="rewardsShop" aria-labelledby="rewards-heading">
      ...
      <div className="rewardRail">{state.rewards.map((reward) => { const limitOwner = reward.scope === "family" ? "family" : rewardKid?.id ?? ""; const remaining = rewardKid ? rewardRemaining(reward, limitOwner) : 0; const quantity = Math.min(remaining, Math.max(1, rewardQuantities[reward.id] ?? 1)); const total = reward.cost * quantity; const shares = Object.fromEntries(state.members.map((member, index) => [member.id, Math.floor(total / state.members.length) + (index < total % state.members.length ? 1 : 0)])); const affordable = reward.scope === "family" ? remaining > 0 && state.members.every((member) => (pointsByMember.find((item) => item.id === member.id)?.points ?? 0) >= shares[member.id]) : Boolean(rewardKid && rewardKid.points >= total && remaining > 0); return <article className={`rewardCard ${reward.scope === "family" ? "familyReward" : ""}`} key={reward.id}>{isParent && <button className="rewardEditButton" onClick={() => { setEditingReward(reward); setShowRewardEditor(true); }} aria-label={`Edit ${reward.title}`}>✎ Edit reward</button>}<span>{reward.emoji}</span><div><h3>{reward.title}</h3><p>{reward.detail}</p><small className="rewardScope">{reward.scope === "family" ? "👨‍👩‍👧 Everyone contributes" : `👤 ${rewardKid?.name ?? "One child"} redeems`}</small>{reward.scope === "family" && <div className="contributionShares">{state.members.map((member) => <span key={member.id} style={{ color: member.color }}>{member.initial}: ⭐ {shares[member.id]}</span>)}</div>}<small className="rewardLimit">{reward.limit === "unlimited" || !reward.limit ? "Unlimited redemptions" : `${remaining} of ${reward.limitQuantity ?? 1} left ${reward.limit}`}</small></div><div className="rewardRedeem"><label>Quantity<select value={quantity} disabled={remaining === 0} onChange={(event) => setRewardQuantities({ ...rewardQuantities, [reward.id]: Number(event.target.value) })}>{Array.from({ length: Math.min(10, remaining) }, (_, index) => index + 1).map((value) => <option key={value} value={value}>{value}</option>)}</select></label><button disabled={!affordable} onClick={() => redeemReward(reward)}>{affordable ? `Request · ⭐ ${total}` : remaining === 0 ? "Limit reached" : reward.scope === "family" ? "Everyone needs more stars" : `Need ⭐ ${total}`}</button></div></article>; })}</div>
