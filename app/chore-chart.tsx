...
type Routine = "morning" | "school" | "afternoon" | "evening" | "anytime";
type Verification = "none" | "parent" | "photo" | "sibling";
type Room = "bedroom" | "bathroom" | "kitchen" | "playroom" | "outside" | "family";
type Chore = { id: string; title: string; detail: string; icon: string; points: number; steps?: string[]; rotate?: boolean; rotationOffset?: number; teamBonus?: number; teamMode?: "one" | "everyone"; roles?: Record<string, string>; verification?: Verification; memberId: string; memberIds?: string[]; cadence: Cadence; routine?: Routine; startTime?: string; endTime?: string; area?: Room; beforeAfter?: boolean; dueDay?: number; dueDate?: number; weeklyGoal?: number };
type Completion = { id?: string; choreId: string; date: string; status?: "pending" | "approved"; proofPath?: string; participantIds?: string[]; stepIds?: string[] };
...
type CalendarEvent = { id: string; title: string; start: string; end: string; allDay: boolean; location: string; calendar: string; type: string; color: string; emoji?: string; manual?: boolean };
...
const badgeCatalog = [
  ...
  { id: "school-ready", emoji: "🏫", name: "Ready-to-Learn Ranger", detail: "Complete 7 school responsibilities", target: 7, kind: "school" },
  { id: "assignment-ace", emoji: "📝", name: "Assignment Adventure Ace", detail: "Complete 15 school responsibilities", target: 15, kind: "school" },
...
const coreChores = [
  ...
  { slug: "school-ready", title: "Get ready for school", detail: "Get dressed, check the weather, and be ready on time", icon: "🏫", points: 10, routine: "school" as Routine, startTime: "06:30", endTime: "08:30" },
  { slug: "homework", title: "Complete homework or study time", detail: "Finish today’s work or practice a skill", icon: "📚", points: 15, routine: "school" as Routine, startTime: "15:00", endTime: "19:00" },
  { slug: "assignments", title: "Check and turn in assignments", detail: "Review what is due and submit completed work", icon: "📝", points: 10, routine: "school" as Routine, startTime: "15:00", endTime: "19:00" },
  { slug: "school-bag", title: "Pack and check your school bag", detail: "Pack papers, supplies, lunch, and your water bottle", icon: "🎒", points: 8, routine: "school" as Routine, startTime: "18:00", endTime: "20:30" },
  { slug: "attendance", title: "Be ready and on time for school", detail: "Celebrate a prepared, on-time school day", icon: "⏰", points: 8, routine: "school" as Routine, verification: "parent" as Verification },
  { slug: "learning-win", title: "Share a grade or learning win", detail: "Celebrate effort, improvement, or a proud school moment", icon: "🌟", points: 20, routine: "school" as Routine, cadence: "flexible" as Cadence, weeklyGoal: 1, verification: "parent" as Verification },
] as const;
...
const routineTemplates = [
  { id: "school", emoji: "🎒", name: "School morning", detail: "A calm, complete get-ready routine", chores: [{ title: "Get ready for school", detail: "Follow each step before heading out", icon: "🌞", points: 20, routine: "morning" as Routine, steps: ["Get dressed", "Brush or style hair", "Fill water bottle", "Pack backpack"] }] },
  { id: "weekend", emoji: "☀️", name: "Weekend morning", detail: "Start the weekend feeling ready", chores: [{ title: "Weekend morning reset", detail: "Finish the basics, then enjoy the day", icon: "☀️", points: 15, routine: "morning" as Routine, steps: ["Get dressed", "Brush teeth", "Make bed", "Put pajamas away"] }] },
  { id: "bedtime", emoji: "🌙", name: "Bedtime", detail: "One friendly evening checklist", chores: [{ title: "Get ready for bed", detail: "Complete each cozy bedtime step", icon: "🌙", points: 20, routine: "evening" as Routine, steps: ["Put on pajamas", "Put clothes in hamper", "Brush teeth", "Pick up bedroom floor", "Choose a book"] }] },
  { id: "visit", emoji: "🧳", name: "Kid visit", detail: "Pack, check, and prepare together", chores: [{ title: "Pack for the visit", detail: "Use the family calendar and checklist", icon: "🧳", points: 20, routine: "anytime" as Routine, steps: ["Pack clothes", "Pack hygiene items", "Pack comfort item", "Check chargers and activities"] }] },
  { id: "vacation", emoji: "🏖️", name: "Vacation", detail: "A lighter travel-ready routine", chores: [{ title: "Vacation ready", detail: "Help the family get ready to go", icon: "🏖️", points: 20, routine: "anytime" as Routine, steps: ["Pack personal bag", "Choose travel activity", "Fill water bottle", "Tidy bedroom before leaving"] }] },
  { id: "sick", emoji: "🍵", name: "Sick day", detail: "Gentle care without streak pressure", chores: [{ title: "Take care of yourself", detail: "Rest and complete only what feels okay", icon: "🍵", points: 10, routine: "anytime" as Routine, steps: ["Drink water", "Take medicine with a parent", "Choose a quiet activity", "Rest"] }] },
  { id: "meeting", emoji: "🤫", name: "Dad has meetings", detail: "Quiet choices while Dad is working", chores: [{ title: "Meeting-time helper", detail: "Choose quiet activities and help the house stay calm", icon: "🤫", points: 15, routine: "afternoon" as Routine, steps: ["Choose a quiet activity", "Use headphones if needed", "Keep toys in one area", "Ask for help between meetings"] }] },
  { id: "rotation", emoji: "🔄", name: "Weekly helper rotation", detail: "Fairly rotate shared household jobs", chores: [{ title: "Set or clear the table", detail: "This job rotates to a different helper each week", icon: "🍽️", points: 12, routine: "evening" as Routine, rotate: true }, { title: "Help with family laundry", detail: "This job rotates to a different helper each week", icon: "🧺", points: 15, routine: "afternoon" as Routine, rotate: true }, { title: "Dinner helper", detail: "This job rotates to a different helper each week", icon: "🥗", points: 15, routine: "evening" as Routine, rotate: true }] },
] as const;
...
function normalizeState(saved: AppState): AppState {
  ...
  const coreSlug = (title: string) => {
    ...
    if (value === "get ready for school") return "school-ready";
    if (value.includes("complete homework") || value === "do homework") return "homework";
    if (value.includes("turn in assignments") || value.includes("assignments turned in")) return "assignments";
    if (value.includes("pack and check your school bag")) return "school-bag";
    if (value.includes("ready and on time for school") || value.includes("good attendance")) return "attendance";
    if (value.includes("grade or learning win") || value.includes("get good grades")) return "learning-win";
    return null;
  };
  ...
  for (const member of members) for (const core of coreChores) if (!removedDefaultChoreIds.includes(`${member.id}-${core.slug}`)) chores.push({ id: `${member.id}-${core.slug}`, title: core.title, detail: core.detail, icon: core.icon, points: core.points, routine: core.routine, memberId: member.id, cadence: "cadence" in core ? core.cadence : "daily", startTime: "startTime" in core ? core.startTime : undefined, endTime: "endTime" in core ? core.endTime : undefined, verification: "verification" in core ? core.verification : undefined, weeklyGoal: "weeklyGoal" in core ? core.weeklyGoal : undefined });
...
const routineLabel = (routine: Routine = "anytime") => routine === "morning" ? "☀️ Morning" : routine === "school" ? "🏫 School" : routine === "afternoon" ? "🎒 After school" : routine === "evening" ? "🌙 Evening" : "✨ Anytime";
...
export function ChoreChart() {
  ...
  const [familyHubTab, setFamilyHubTab] = useState<"weather" | "calendar" | "rewards" | "quest">("weather");
  const [showEventEditor, setShowEventEditor] = useState(false);
  const [editingFamilyEvent, setEditingFamilyEvent] = useState<CalendarEvent | null>(null);
  ...
  const monthDates = useMemo(() => { const first = new Date(selectedDate.getFullYear(), selectedDate.getMonth(), 1); const gridStart = startOfWeek(first); return Array.from({ length: 42 }, (_, index) => addDays(gridStart, index)); }, [selectedDate]);
  const calendarDates = useMemo(() => calendarView === "day" ? [selectedDate] : calendarView === "week" ? weekDates : monthDates, [calendarView, selectedDate, weekDates, monthDates]);
  const eventsOn = (date: Date) => calendarEvents.filter((event) => iso(new Date(event.start)) === iso(date));
  const calendarTitle = calendarView === "day" ? selectedDate.toLocaleDateString([], { weekday: "long", month: "long", day: "numeric" }) : calendarView === "week" ? `${weekDates[0].toLocaleDateString([], { month: "short", day: "numeric" })} – ${weekDates[6].toLocaleDateString([], { month: "short", day: "numeric", year: "numeric" })}` : selectedDate.toLocaleDateString([], { month: "long", year: "numeric" });
  const moveCalendar = (direction: number) => setSelectedDate(calendarView === "day" ? addDays(selectedDate, direction) : calendarView === "week" ? addDays(selectedDate, direction * 7) : new Date(selectedDate.getFullYear(), selectedDate.getMonth() + direction, 1));
  const rotationMemberId = (chore: Chore, date: Date | string = selectedDate) => { if (!chore.rotate) return chore.memberId; const when = typeof date === "string" ? new Date(`${date}T12:00:00`) : date; const week = Math.floor(Date.UTC(when.getFullYear(), when.getMonth(), when.getDate()) / 604800000); return state.members[(week + (chore.rotationOffset ?? 0)) % state.members.length]?.id ?? chore.memberId; };
  const choreBelongsTo = (chore: Chore, memberId: string, date: Date | string = selectedDate) => chore.memberIds?.includes(memberId) || rotationMemberId(chore, date) === memberId;
  useEffect(() => {
    const from = calendarDates[0].toISOString(); const to = addDays(calendarDates[calendarDates.length - 1], 1).toISOString();
  ...
  }, [calendarDates, calendarRefresh]);
  const visibleChores = state.chores.filter((chore) => {
    const memberMatches = activeMember === "all" || choreBelongsTo(chore, activeMember);
  ...
  const isComplete = (choreId: string, date = selectedIso) => { const completion = state.completions.find((item) => item.choreId === choreId && item.date === date); const chore = state.chores.find((item) => item.id === choreId); return Boolean(completion && (!chore?.steps?.length || (completion.stepIds?.length ?? 0) >= chore.steps.length)); };
  ...
  const toggleChecklistStep = (chore: Chore, stepIndex: number, date = selectedIso) => {
    const existing = state.completions.find((item) => item.choreId === chore.id && item.date === date);
    const stepIds = new Set(existing?.stepIds ?? []); const stepId = String(stepIndex);
    if (stepIds.has(stepId)) stepIds.delete(stepId); else stepIds.add(stepId);
  ...
  const activateTemplate = (template: typeof routineTemplates[number]) => {
    const stamp = Date.now(); const chores: Chore[] = [];
    template.chores.forEach((item, itemIndex) => { const steps = "steps" in item ? [...item.steps] : undefined; if ("rotate" in item && item.rotate) chores.push({ ...item, steps, id: `template-${template.id}-${stamp}-${itemIndex}`, memberId: state.members[0].id, cadence: "daily", rotationOffset: itemIndex }); else state.members.forEach((member, memberIndex) => chores.push({ ...item, steps, id: `template-${template.id}-${stamp}-${itemIndex}-${memberIndex}`, memberId: member.id, cadence: "daily" })); });
    persist({ ...state, chores: [...state.chores, ...chores] }); setShowTemplates(false); setCelebration({ emoji: template.emoji, color: "#6957d5", name: template.name, message: "Routine activated:" }); window.setTimeout(() => setCelebration(null), 1700);
  };
  ...
  const pointsByMember = state.members.map((member) => {
    ...
    state.completions.filter((item) => item.date >= pointPeriodStart && item.status !== "pending").forEach((item) => {
      const chore = state.chores.find((entry) => entry.id === item.choreId && choreBelongsTo(entry, member.id, item.date));
  ...
  const calendarSuggestions = Array.from(new Set(calendarEvents.flatMap((event) => { const context = `${event.calendar} ${event.type} ${event.title}`.toLowerCase(); if (/visit|custody|overnight|travel/.test(context)) return ["Pack overnight bag"]; if (/meeting|work|call|conference/.test(context)) return ["Choose a quiet activity"]; if (/sport|soccer|baseball|practice|game/.test(context)) return ["Prepare sports gear"]; if (/birthday|party/.test(context)) return ["Help prepare for the celebration"]; if (/school|class|field trip/.test(context)) return ["Pack tomorrow’s backpack"]; return []; })));
  ...
  const addChore = (form: FormData) => {
    ...
    const base = {
      ...
      icon: String(form.get("icon") || "✨"), points: Number(form.get("points")) || 5, steps: String(form.get("steps") || "").split("\n").map((step) => step.trim()).filter(Boolean), rotate: assignee !== "team" && assignee !== "all" && form.get("rotate") === "on", teamBonus: assignee === "team" ? Math.max(0, Number(form.get("teamBonus")) || 0) : undefined, teamMode: assignee === "team" ? String(form.get("teamMode") || "everyone") as "one" | "everyone" : undefined, roles: assignee === "team" ? Object.fromEntries(state.members.map((member) => [member.id, String(form.get(`role-${member.id}`) || "Help the team")])) : undefined, verification: String(form.get("verification") || "none") as Verification,
  ...
  const addCalendarSuggestion = (title: string) => {
    const template = title === "Pack overnight bag" ? { icon: "🎒", detail: "Before the upcoming visit", points: 12 } : title === "Prepare sports gear" ? { icon: "⚽", detail: "Before the upcoming practice or game", points: 10 } : title === "Help prepare for the celebration" ? { icon: "🎉", detail: "Before the upcoming birthday or party", points: 10 } : title === "Pack tomorrow’s backpack" ? { icon: "🎒", detail: "Before the upcoming school event", points: 10 } : { icon: "🤫", detail: "During an upcoming meeting", points: 8 };
  ...
  const saveFamilyEvent = async (form: FormData) => {
    setEventEditorError(""); const date = String(form.get("date") || selectedIso); const allDay = form.get("allDay") === "on"; const startTime = String(form.get("startTime") || "09:00"); const endTime = String(form.get("endTime") || "10:00");
    const start = new Date(`${date}T${allDay ? "00:00" : startTime}:00`); const end = allDay ? addDays(start, 1) : new Date(`${date}T${endTime}:00`);
    const body = { id: editingFamilyEvent?.id, title: form.get("title"), start: start.toISOString(), end: end.toISOString(), allDay, location: form.get("location"), calendar: form.get("calendar"), color: form.get("color"), emoji: form.get("emoji") };
    const response = await fetch("/api/calendar/events", { method: editingFamilyEvent ? "PATCH" : "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) }); const result = await response.json();
    if (!response.ok) { setEventEditorError(result.error || "The event could not be saved."); return; } setShowEventEditor(false); setEditingFamilyEvent(null); setCalendarRefresh((value) => value + 1);
  };
  const deleteFamilyEvent = async () => { if (!editingFamilyEvent) return; const response = await fetch(`/api/calendar/events?id=${encodeURIComponent(editingFamilyEvent.id)}`, { method: "DELETE" }); if (!response.ok) { setEventEditorError("The event could not be removed."); return; } setShowEventEditor(false); setEditingFamilyEvent(null); setCalendarRefresh((value) => value + 1); };
  ...
  return <main className={`shell ${state.accessibilitySettings.highContrast ? "highContrast" : ""} ${state.accessibilitySettings.largeText ? "largeText" : ""} ${state.accessibilitySettings.reducedMotion ? "reduceMotion" : ""}`}>
    ...
    <nav className="familyHubTabs" aria-label="Family information"><button className={familyHubTab === "weather" ? "active" : ""} onClick={() => setFamilyHubTab("weather")}><span>{weather?.emoji ?? "🌤️"}</span><strong>Weather</strong><small>{weather ? `${weather.temperature}°` : "Local"}</small></button><button className={familyHubTab === "calendar" ? "active" : ""} onClick={() => setFamilyHubTab("calendar")}><span>🗓️</span><strong>Calendar</strong><small>{calendarEvents.length} event{calendarEvents.length === 1 ? "" : "s"}</small></button><button className={familyHubTab === "rewards" ? "active" : ""} onClick={() => setFamilyHubTab("rewards")}><span>🎁</span><strong>Rewards</strong><small>{state.rewards.length} choices</small></button><button className={familyHubTab === "quest" ? "active" : ""} onClick={() => setFamilyHubTab("quest")}><span>{familyQuestPercent >= 100 ? "🎉" : "🗺️"}</span><strong>Weekly Quest</strong><small>{familyQuestPercent}% complete</small></button></nav>

    {familyHubTab === "weather" && <section className="weatherCard hubPanel" aria-labelledby="weather-heading"><div className="weatherNow"><span>{weather?.emoji ?? (weatherError ? "🌈" : "⏳")}</span><div><p className="eyebrow">Right outside</p><h2 id="weather-heading">{weather ? `${weather.temperature}° · ${weather.label}` : weatherError ? "Weather is taking a break" : "Checking the sky…"}</h2><small>{weather ? `${weather.place} · Feels like ${weather.feelsLike}° · Wind ${weather.wind} mph` : weatherError ? "Try refreshing again in a moment." : `ZIP ${state.engagementSettings.weatherZip}`}</small></div></div>{weather && <div className="weatherForecast">{weather.forecast.map((day, index) => <article key={day.date}><span>{day.emoji}</span><div><strong>{index === 0 ? "Today" : new Date(`${day.date}T12:00:00`).toLocaleDateString([], { weekday: "short" })}</strong><small>{day.label}</small></div><b>{day.high}° <i>{day.low}°</i></b></article>)}</div>}</section>}

    {familyHubTab === "quest" && <section className={`familyQuest hubPanel ${familyQuestPercent >= 100 ? "complete" : ""}`}><span>{familyQuestPercent >= 100 ? "🎉" : "🗺️"}</span><div><p className="eyebrow">Weekly family quest</p><h2>{state.engagementSettings.questEnabled ? familyQuestPercent >= 100 ? `${state.engagementSettings.questReward} unlocked!` : `Reach ${state.engagementSettings.questTarget} family stars` : "Weekly quest is resting"}</h2>{state.engagementSettings.questEnabled ? <><div><i style={{ width: `${familyQuestPercent}%` }} /></div><small>{familyQuestPoints} of {state.engagementSettings.questTarget} stars · Everyone contributes</small></> : <small>A parent can turn the shared quest back on under Fun, quests & family modes.</small>}</div></section>}

    {familyHubTab === "calendar" && <section className="familyCalendar hubPanel" aria-labelledby="calendar-heading">
      <div className="calendarTitle"><div><p className="eyebrow">Family schedule</p><h2 id="calendar-heading">{calendarTitle}</h2></div><div className="calendarHeaderActions">{isParent && <button className="addFamilyEvent" onClick={() => { setEditingFamilyEvent(null); setEventEditorError(""); setShowEventEditor(true); }}>＋ Add event</button>}<div className="calendarViewTabs">{(["day", "week", "month"] as const).map((view) => <button key={view} className={calendarView === view ? "active" : ""} onClick={() => setCalendarView(view)}>{view[0].toUpperCase() + view.slice(1)}</button>)}</div></div></div>
      <div className="calendarToolbar"><div><button onClick={() => moveCalendar(-1)} aria-label={`Previous ${calendarView}`}>‹</button><button onClick={() => setSelectedDate(new Date())}>Today</button><button onClick={() => moveCalendar(1)} aria-label={`Next ${calendarView}`}>›</button></div><div className="calendarLegend">{Array.from(new Map(calendarEvents.map((event) => [event.calendar, event])).values()).map((event) => <span key={event.calendar}><i style={{ background: event.color }} />{event.emoji} {event.calendar}</span>)}</div></div>
      {calendarView === "day" ? <div className="calendarDayView"><div className="calendarDayNumber"><strong>{dayNames[selectedDate.getDay()]}</strong><span>{selectedDate.getDate()}</span></div><div className="calendarDayAgenda">{eventsOn(selectedDate).map((event) => { const start = new Date(event.start); const end = new Date(event.end); return <article key={event.id} style={{ "--event-color": event.color } as React.CSSProperties}><time>{event.allDay ? "All day" : start.toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</time><div><small>{event.emoji} {event.calendar}</small><h3>{event.title}</h3><p>{event.allDay ? "All-day event" : `Until ${end.toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}`}{event.location ? ` · ${event.location}` : ""}</p></div></article>; })}{eventsOn(selectedDate).length === 0 && <p className="calendarNoEvents">No events scheduled for this day.</p>}</div></div>
      : calendarView === "week" ? <div className="calendarWeekView">{weekDates.map((date) => <section key={iso(date)} className={`${iso(date) === iso() ? "today" : ""} ${iso(date) === selectedIso ? "selected" : ""}`} onClick={() => setSelectedDate(date)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") setSelectedDate(date); }} role="button" tabIndex={0}><header><strong>{dayNames[date.getDay()]}</strong><span>{date.getDate()}</span></header><div>{eventsOn(date).map((event) => <article key={event.id} style={{ "--event-color": event.color } as React.CSSProperties}><time>{event.allDay ? "All day" : new Date(event.start).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</time><b>{event.emoji} {event.title}</b><small>{event.calendar}</small></article>)}</div></section>)}</div>
      : <div className="calendarMonthView"><div className="monthWeekdays">{dayNames.map((day) => <span key={day}>{day}</span>)}</div><div className="monthGrid">{monthDates.map((date) => { const dayEvents = eventsOn(date); const inMonth = date.getMonth() === selectedDate.getMonth(); return <button key={iso(date)} className={`${inMonth ? "" : "outside"} ${iso(date) === iso() ? "today" : ""} ${iso(date) === selectedIso ? "selected" : ""}`} onClick={() => { setSelectedDate(date); setCalendarView("day"); }}><span>{date.getDate()}</span><div>{dayEvents.slice(0, 3).map((event) => <i key={event.id} style={{ "--event-color": event.color } as React.CSSProperties}>{event.emoji} {event.title}</i>)}{dayEvents.length > 3 && <small>+{dayEvents.length - 3} more</small>}</div></button>; })}</div></div>}
      {calendarEvents.length === 0 && <div className="calendarEmpty"><span>🗓️</span><div><strong>{calendarConfigured === false ? "Your calendars are ready to connect" : `No events in this ${calendarView}`}</strong><p>{calendarConfigured === false ? "Add a private Google, iCloud, Outlook, or other iCal feed in Parent Mode." : "Looks like open family time!"}</p></div></div>}
    ...
    {familyHubTab === "rewards" && <section className="rewardsShop hubPanel" aria-labelledby="rewards-heading">
    ...
    <section className="dashboard" aria-label="Chore chart">
      ...
      {tab === "today" && <div className="routineFocus" aria-label="Routine"><button className={routineFocus === "now" ? "active now" : ""} onClick={() => setRoutineFocus("now")}>⏰ Due now</button><button className={routineFocus === "all" ? "active" : ""} onClick={() => setRoutineFocus("all")}>All day</button><button className={routineFocus === "morning" ? "active" : ""} onClick={() => setRoutineFocus("morning")}>☀️ Morning</button><button className={routineFocus === "school" ? "active" : ""} onClick={() => setRoutineFocus("school")}>🏫 School</button><button className={routineFocus === "afternoon" ? "active" : ""} onClick={() => setRoutineFocus("afternoon")}>🎒 After school</button><button className={routineFocus === "evening" ? "active" : ""} onClick={() => setRoutineFocus("evening")}>🌙 Bedtime</button></div>}
      ...
      {tab === "today" ? <div className="choreGrid">{visibleChores.map((chore) => {
        const member = state.members.find((entry) => entry.id === rotationMemberId(chore))!; const count = chore.cadence === "flexible" ? flexibleCount(chore.id) : 0; const done = chore.cadence === "flexible" ? count >= (chore.weeklyGoal ?? 1) : isComplete(chore.id);
      ...
      : tab === "week" ? <div className="weeklyTable"><div className="weeklyHead"><span>Chore</span>{weekDates.map((date) => <span key={iso(date)}>{dayNames[date.getDay()]}</span>)}</div>{visibleChores.map((chore) => {
        const member = state.members.find((entry) => entry.id === rotationMemberId(chore))!;
    ...
    {showAdd && <div className="modalBackdrop" onMouseDown={(event) => event.target === event.currentTarget && setShowAdd(false)}><form className="modal" action={addChore}>
      ...
      <label>Routine checklist steps<textarea name="steps" rows={4} placeholder={"Optional — one step per line\nGet dressed\nFill water bottle\nPack backpack"} /></label><p className="fieldHint">Kids can tap each step. The whole routine finishes when every step is checked.</p>
      ...
      <div className="formRow"><label>Assigned to<select name="memberId"><option value="all">Everyone — separately</option><option value="team">🤝 Team chore — together</option>{state.members.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</select></label><label>Routine<select name="routine" defaultValue="anytime"><option value="morning">Morning</option><option value="school">School</option><option value="afternoon">After school</option><option value="evening">Evening</option><option value="anytime">Anytime</option></select></label></div>
      <label className="toggleField"><input name="rotate" type="checkbox" /> Rotate this job to the next child every week</label><p className="fieldHint">Choose one starting child above. Tidy Team will rotate the assignment fairly each week.</p>
    ...
    {editingChore && <div className="modalBackdrop" onMouseDown={(event) => event.target === event.currentTarget && setEditingChore(null)}><form className="modal" action={saveChore}>
      ...
      <label>Routine checklist steps<textarea name="steps" rows={4} defaultValue={editingChore.steps?.join("\n") ?? ""} placeholder="Optional — one step per line" /></label><p className="fieldHint">Each step appears as a large kid-friendly tap target.</p>
      ...
      <div className="formRow"><label>Assigned to<select name="memberId" defaultValue={editingChore.memberIds?.length ? "team" : editingChore.memberId}><option value="team">🤝 Team chore — together</option>{state.members.map((m) => <option key={m.id} value={m.id}>{m.name}</option>)}</select></label><label>Routine<select name="routine" defaultValue={editingChore.routine ?? "anytime"}><option value="morning">Morning</option><option value="school">School</option><option value="afternoon">After school</option><option value="evening">Evening</option><option value="anytime">Anytime</option></select></label></div>
      <label className="toggleField"><input name="rotate" type="checkbox" defaultChecked={editingChore.rotate} /> Rotate this job to the next child every week</label><p className="fieldHint">Rotation applies to an individual chore, not a team chore.</p>
    ...
    {showEventEditor && isParent && (() => { const start = new Date(editingFamilyEvent?.start ?? `${selectedIso}T09:00:00`); const end = new Date(editingFamilyEvent?.end ?? `${selectedIso}T10:00:00`); return <div className="modalBackdrop" onMouseDown={(event) => event.target === event.currentTarget && setShowEventEditor(false)}><form className="modal familyEventEditor" action={saveFamilyEvent}><button type="button" className="close" onClick={() => setShowEventEditor(false)} aria-label="Close">×</button><p className="eyebrow">Petrous Family calendar</p><h2>{editingFamilyEvent ? "Edit family event" : "Add an event"}</h2><label>Event title<input name="title" defaultValue={editingFamilyEvent?.title ?? ""} placeholder="Soccer practice, Grandma visits…" autoFocus required /></label><div className="eventIdentityRow"><label>Icon<input name="emoji" defaultValue={editingFamilyEvent?.emoji ?? "📌"} maxLength={8} /></label><label>Calendar or category<input name="calendar" defaultValue={editingFamilyEvent?.calendar ?? "Petrous Family"} maxLength={40} /></label><label>Color<input name="color" type="color" defaultValue={editingFamilyEvent?.color ?? "#6957d5"} /></label></div><label>Date<input name="date" type="date" defaultValue={iso(start)} required /></label><label className="toggleField"><input name="allDay" type="checkbox" defaultChecked={editingFamilyEvent?.allDay} /> All-day event</label><div className="formRow"><label>Starts<input name="startTime" type="time" defaultValue={start.toTimeString().slice(0, 5)} /></label><label>Ends<input name="endTime" type="time" defaultValue={end.toTimeString().slice(0, 5)} /></label></div><label>Location or helpful note<input name="location" defaultValue={editingFamilyEvent?.location ?? ""} placeholder="Optional" /></label>{eventEditorError && <p className="pinError" role="alert">{eventEditorError}</p>}<div className="modalActions">{editingFamilyEvent && <button type="button" className="deleteButton" onClick={deleteFamilyEvent}>Delete event</button>}<button className="saveButton" type="submit">{editingFamilyEvent ? "Save event" : "Add to calendar"}</button></div>{!editingFamilyEvent && calendarEvents.some((event) => event.manual) && <div className="manageFamilyEvents"><strong>Your events in this view</strong>{calendarEvents.filter((event) => event.manual).map((event) => <button type="button" key={event.id} onClick={() => setEditingFamilyEvent(event)}><span style={{ background: event.color }}>{event.emoji}</span><span><b>{event.title}</b><small>{new Date(event.start).toLocaleDateString()} · {event.calendar}</small></span><i>✎</i></button>)}</div>}</form></div>; })()}
