...
type Reward = { id: string; title: string; detail: string; emoji: string; cost: number; limit?: RewardLimit; limitQuantity?: number };
type Redemption = { id: string; rewardId: string; rewardTitle: string; memberId: string; cost: number; quantity?: number; redeemedAt: string; status?: "pending" | "approved" };
...
export function ChoreChart() {
  ...
  const redeemReward = (reward: Reward) => {
    ...
    if (!rewardKid || rewardKid.points < totalCost || rewardRemaining(reward, rewardKid.id) < quantity) return;
    const redemption: Redemption = { id: `${Date.now()}`, rewardId: reward.id, rewardTitle: reward.title, memberId: rewardKid.id, cost: totalCost, quantity, redeemedAt: new Date().toISOString(), status: "pending" };
  ...
  const addReward = (form: FormData) => {
    const title = String(form.get("title") || "").trim();
    if (!title) return;
    const reward: Reward = { id: editingReward?.id ?? `${Date.now()}`, title, detail: String(form.get("detail") || "A custom family reward").trim(), emoji: String(form.get("emoji") || "🎁"), cost: Math.max(1, Number(form.get("cost")) || 25), limit: String(form.get("limit") || "unlimited") as RewardLimit, limitQuantity: Math.max(1, Number(form.get("limitQuantity")) || 1) };
  ...
  return <main className={`shell ${state.accessibilitySettings.highContrast ? "highContrast" : ""} ${state.accessibilitySettings.largeText ? "largeText" : ""} ${state.accessibilitySettings.reducedMotion ? "reduceMotion" : ""}`}>
    ...
    <section className="rewardsShop" aria-labelledby="rewards-heading">
      ...
      <div className="rewardRail">{state.rewards.map((reward) => { const remaining = rewardKid ? rewardRemaining(reward, rewardKid.id) : 0; const quantity = Math.min(remaining, Math.max(1, rewardQuantities[reward.id] ?? 1)); const affordable = Boolean(rewardKid && rewardKid.points >= reward.cost * quantity && remaining > 0); return <article className="rewardCard" key={reward.id}>{isParent && <button className="rewardEditButton" onClick={() => { setEditingReward(reward); setShowRewardEditor(true); }} aria-label={`Edit ${reward.title}`}>✎ Edit reward</button>}<span>{reward.emoji}</span><div><h3>{reward.title}</h3><p>{reward.detail}</p><small className="rewardLimit">{reward.limit === "unlimited" || !reward.limit ? "Unlimited redemptions" : `${remaining} of ${reward.limitQuantity ?? 1} left ${reward.limit}`}</small></div><div className="rewardRedeem"><label>Quantity<select value={quantity} disabled={remaining === 0} onChange={(event) => setRewardQuantities({ ...rewardQuantities, [reward.id]: Number(event.target.value) })}>{Array.from({ length: Math.min(10, remaining) }, (_, index) => index + 1).map((value) => <option key={value} value={value}>{value}</option>)}</select></label><button disabled={!affordable} onClick={() => redeemReward(reward)}>{affordable ? `Redeem · ⭐ ${reward.cost * quantity}` : remaining === 0 ? "Limit reached" : `Need ⭐ ${reward.cost * quantity}`}</button></div></article>; })}</div>
